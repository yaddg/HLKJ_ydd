...
var set_cookie = null;
var zjtDomainUrl = "http://maoqiankeji.cn/zhijt-activity";//生产环境
var registerObj = {
	downloadUrl: null,
	loadVerCode: function() {
		...
		$.ajax({
			url: zjtDomainUrl+"/api/member/h5api/sendSmscode",
			data: {
				userid: userid,
				sign: sign
			},
...
//		}
		...
		MD5STR = md5(MD5STR); 
		$.ajax({
			url: zjtDomainUrl+"/api/member/h5api/register",
			data: {
				userid: userid,
				password: pwd,
				smsCode: verCode,
			...
			success: function(data, status, xhr) {
				...
				if(data.code == 0) {
					//alert("注册成功！") 
					window.location.href = 'http://qr32.cn/EuTvYq';
				} else if(data.code == 500) {
