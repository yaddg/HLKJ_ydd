LazyLoad = function(x, h) {
	function r(b, a) {
		var c = h.createElement(b),
			d;
		for(d in a) a.hasOwnProperty(d) && c.setAttribute(d, a[d]);
		return c
	}
	...
	function w() {
		if(!e) {
			...
			e = {
				async: h.createElement("script").async === true
			};
	...
	function m(b,
		...
		if(a) {
			...
			if(o || e.async || e.gecko || e.opera) j[b].push({
				urls: a,
				callback: c,
				obj: d,
				context: s
				...
				f = 0;
				for(g = a.length; f < g; ++f) j[b].push({
					urls: [a[f]],
					callback: f === g - 1 ? c : null,
					obj: d,
					context: s
				})
			}
		...
		if(!(i[b] || !(p = i[b] = j[b].shift()))) {
			...
			for(g = a.length; f < g; ++f) {
				...
				if(e.ie) c.onreadystatechange = function() {
					...
					if(e.webkit) {
						p.urls[f] = c.href;
						u()
					} else setTimeout(n, 50 * g);
				else c.onload = c.onerror = n;
				q.appendChild(c)
			}
		}
	}
	...
	function u() {
		...
		if(b) {
			...
			l += 1;
			if(b) l < 200 ? setTimeout(u, 50) : k("css")
		}
	}
	var e, q, i = {},
		l = 0,
