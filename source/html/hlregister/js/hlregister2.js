...
window.onload = function() { 
	...
	$("#verCodeInput").css("width", inputWidth + "px");
		...
		$("#invitecode").val(rqstObj.invitecode);
		...
		//1.计算宽度的比例
		//var whratio = htmlWidth / 图片宽度;

		//2.计算form显示位置
		//var formTop = htmlWidth * whratio;
		var formDivTop = htmlWidth - 18;
		$("#fromDiv").css('top', formDivTop + 'px');

		var verdivWidth = (htmlWidth - 20) * 0.6;
		$("#formVerCodeDiv").css("width", verdivWidth + "px"); //获取验证码 背景

		var imgVerCodeObjWidth = (htmlWidth - 20) * 0.4 - 8;
		var imgVerCodeObjRight = imgVerCodeObjWidth + 8;
		$("#getVerCodeBut").css("width", imgVerCodeObjWidth + "px");
		$("#getVerCodeBut").css("right", -imgVerCodeObjRight + "px");
